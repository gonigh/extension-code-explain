...
const appendChat = function (msg, role) {
	...
	chat.innerHTML = `<div class="chat-item ${_role}">
                        <div class="userinfo">
...
;(function () {
	...
	const copyCode = codeElement => {
		...
		codeElement.parentNode.querySelectorAll(".copy-success")[0].style.display =
			"flex"
		setTimeout(() => {
			codeElement.parentNode.querySelectorAll(".copy")[0].style.display = "flex"
			codeElement.parentNode.querySelectorAll(
				".copy-success"
	...
	const insetCode = (codeElement, type) => {
		...
		try {
			...
			sendTA(
				`${TA_PREFIX}_${type === "newfile" ? "insertNewFile" : "insert"}.click`
			)
		} catch (e) {
			console.log(e)
		}
	...
	window.addEventListener("message", event => {
		...
		if (messageStrategy[message.type]) {
			...
			cs.run(JSON.parse(event.data.value), "Hexin Copilot")
		}
	})
